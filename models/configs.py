...
from .layers import dilated_causal_conv1d, dilated_causal_conv1d_no_pad, fast_gen_dilated_causal_conv1d, self_gated_layer
    ...
    def build(self, inputs, reuse=None):
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            with tf.variable_scope("softmax"):
                ...
                modes = tf.argmax(scores, axis=-1) - self.central_class
...
class FastGenV1(object):
    ...
    def build(self, inputs, reuse=None):
        with tf.variable_scope(self.name, reuse=reuse):
            wav = inputs["wav"]
            assert wav.get_shape().with_rank(2)
            assert wav.get_shape()[1].value == 1
            mu_wav = mu_law(wav)
            wav_indices = tf.cast(mu_wav + self.central_class, dtype=tf.int32)
            ...
            with tf.variable_scope("init_causal_layer"):
                ...
                # 3rd. calculate
                init_causal_out = fast_gen_dilated_causal_conv1d(inputs=wav_one_hot, dilation_rate=1,
            ...
            return {"init_op": init_op_lst, "synthesized_samples": synthesized_samples}


class FastGenV2(object):
    def __init__(self, name="WaveNet"):
        self.name = name
        self.max_global_steps = 200000
        self.blks = 3
